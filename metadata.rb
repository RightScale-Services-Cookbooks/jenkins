...
recipe "jenkins::default"                 , "Install the dependencies for jenkins."
recipe "jenkins::install_server"          , "Install Jenkins server and configure it using the inputs provided."
recipe "jenkins::do_attach_request"       , "Attach a slave node to the master Jenkins server."
recipe "jenkins::do_attach_slave_at_boot" , "Attach a slave node to the master Jenkins server at boot time if 'jenkins/attach_slave_at_boot' is set to true"
...
attribute "jenkins/server/version",
  ...
  :description  => "Jenkins version to install. Default 1.500. Following also work 1.520, 1.525, 1.535 Example: 1.500",
  :required     => "optional",
  :default      => "1.500",
  :recipes      => ["jenkins::default", "jenkins::install_server"]
